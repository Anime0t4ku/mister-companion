...
ONLINE_GAME_ROOTS = [("SD Card", "/media/fat/games")]
ONLINE_ARCADE_ROOTS = [("SD Card", "/media/fat/_Arcade")]
...
SYSTEM_PRESETS["MiSTerDVD"] = {
    "aliases": ["MiSTer DVD", "DVD"],
    "variants": [
        {
            "role": "video",
            "label": "Video",
...
def _online_game_roots(connection, arcade=False):
    folder = "_Arcade" if arcade else "games"
    candidates = [("SD Card", f"/media/fat/{folder}")]
    if not arcade:
        candidates.append(("Network / CIFS", "/media/fat/cifs/games"))

    for root in available_roots(connection):
        root_path = str(root.get("path") or "")
        if re.fullmatch(r"/media/usb\d+", root_path):
            candidates.append((str(root.get("name") or "USB Drive"), f"{root_path}/{folder}"))

    sftp = connection.client.open_sftp()
    try:
        return [(label, path) for label, path in candidates if _sftp_exists(sftp, path)]
    finally:
        sftp.close()
...
class CollectionLauncherManagerDialog(QDialog):
    ...
    def browse_game(self, extensions, arcade=False):
        ...
        dlg=RemoteGameBrowserDialog(self.connection,extensions,self,roots=_online_game_roots(self.connection, arcade=arcade))
        return dlg.selected_path if dlg.exec()==QDialog.DialogCode.Accepted else ''
