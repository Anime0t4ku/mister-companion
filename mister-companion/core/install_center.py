...
from core.extras_mister_duke3d import (
    get_mister_duke3d_status,
    get_mister_duke3d_status_local,
    install_or_update_mister_duke3d,
    install_or_update_mister_duke3d_local,
    uninstall_mister_duke3d,
    uninstall_mister_duke3d_local,
)
...
from core.extras_misterzine_frontend import (
    get_misterzine_frontend_status,
    get_misterzine_frontend_status_local,
    install_or_update_misterzine_frontend,
    install_or_update_misterzine_frontend_local,
    uninstall_misterzine_frontend,
    uninstall_misterzine_frontend_local,
)
...
DOWNLOADER_HANDLER_DATABASES = {
    ...
    "misterzine_frontend": "misterzine",
}
...
def _downloader_database_url(db_id: str) -> str | None:
    ...
    if db_id == "misterzine":
        return "https://github.com/matijaerceg/misterzine-on-device/releases/latest/download/misterzine.json.zip"
    return None
...
FALLBACK_ITEMS = [
    ...
    ("misterzine_frontend", "extras", "extra", "misterzine_frontend", "MiSTerZine Frontend", "Matija Erceg", "Browse MiSTer cores and arcade releases directly on your MiSTer, with release tracking, screenshots, installed/update status, and game launching."),
...
EXTRA_HANDLERS = {
    ...
    "mister_duke3d": (get_mister_duke3d_status, get_mister_duke3d_status_local, install_or_update_mister_duke3d, install_or_update_mister_duke3d_local, uninstall_mister_duke3d, uninstall_mister_duke3d_local),
    ...
    "misterzine_frontend": (get_misterzine_frontend_status, get_misterzine_frontend_status_local, install_or_update_misterzine_frontend, install_or_update_misterzine_frontend_local, uninstall_misterzine_frontend, uninstall_misterzine_frontend_local),
}
...
def run_uninstall(item: dict, context: InstallCenterContext, log: Callable[[str], None], force_downloader: bool = False):
    ...
    if item_type in {"extra", "core"} or category in {"extras", "cores"}:
        ...
        if handler in {"retroachievement_cores", "3s_arm", "3sx_mister", "dreamster", "mister_duke3d", "mister_quake", "mms2_gb_core", "paprium_megadrive", "sonic_mania_mister", "megavgmdrive", "physical_disc_cores", "solarus", "mister_dvd", "dvd_player", "misterzine_frontend"}:
            if context.offline:
                uninstall_local(context.sd_root, log, force=force_downloader)
            else:
                uninstall_online(context.connection, log, force=force_downloader)
